...
const PrismBg = ({
  ...
  useEffect(() => {
    ...
    const setMat3FromEuler = (yawY, pitchX, rollZ, out) => {
      ...
      out[8] = cy * cx;
      // OGL mat3 is column-major
      const t0 = out[1]; out[1] = out[3]; out[3] = t0;
      const t1 = out[2]; out[2] = out[6]; out[6] = t1;
      const t2 = out[5]; out[5] = out[7]; out[7] = t2;
      return out;
    };
    ...
    if (animationType === 'hover') {
      ...
      // Keep base wobble enabled for ambient motion
    ...
    // Ambient drift parameters (subtle idle animation)
    const ambientSpeed = 0.15; // very slow
    const ambientAmplitude = 0.12; // subtle angle range
    ...
    const render = (t) => {
      ...
      if (animationType === 'hover') {
        // Ambient sinusoidal drift (always active)
        const ambientYaw = Math.sin(time * ambientSpeed) * ambientAmplitude;
        const ambientPitch = Math.cos(time * ambientSpeed * 0.7 + 1.0) * ambientAmplitude * 0.6;
        const ambientRoll = Math.sin(time * ambientSpeed * 0.5 + 2.0) * ambientAmplitude * 0.3;

        // Hover target (adds on top of ambient)
        const maxPitch = 0.6 * HOVSTR;
        const maxYaw = 0.6 * HOVSTR;
        const hoverYaw = (pointer.inside ? -pointer.x : 0) * maxYaw;
        const hoverPitch = (pointer.inside ? pointer.y : 0) * maxPitch;

        // Smooth lerp toward combined target
        targetYaw = ambientYaw + hoverYaw;
        targetPitch = ambientPitch + hoverPitch;
        yaw = lerp(yaw, targetYaw, INERT);
        pitch = lerp(pitch, targetPitch, INERT);
        roll = lerp(roll, ambientRoll, 0.05);

        program.uniforms.uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf);
        // Always continue RAF — ambient motion never stops
      } else if (animationType === '3drotate') {
  ...
  return <div className="prism-bg-container" ref={containerRef} />;
};

export default PrismBg;
