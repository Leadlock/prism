...
- `SMTP_HOST`: Email server host (optional; default `smtp.gmail.com`)
- `SMTP_PORT`: Email server port (optional; default `587`)
- `SMTP_SECURE`: `true` for port 465, `false` for 587/STARTTLS (optional; default `false`)
- `SMTP_USER`: Email username (optional; e.g. `team@prismgrc.co`)
- `SMTP_PASSWORD`: Email password (optional; for Gmail this is a **16-char App Password**, requires 2FA on the account)
- `EMAIL_FROM`: From address for emails (optional; must equal `SMTP_USER` or a Gmail verified alias, else Gmail rewrites it)
- `TEAM_NOTIFY_EMAIL`: Internal address notified when a company finishes the self-assessment (optional; default `team@prismgrc.co`)
